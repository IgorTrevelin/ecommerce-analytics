...
def clear_database(conn: Connection):
    with open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "schema.sql"), "r") as f:
        queries = f.read()
...
def load_initial_dataset(conn: Connection):
    # load data
    df_categories = load_csv("product_category_name_translation.csv", usecols=["product_category_name", "product_category_name_english"])
    df_zip = load_csv("olist_geolocation_dataset.csv", usecols=["geolocation_zip_code_prefix", "geolocation_state"])
    df_products = load_csv("olist_products_dataset.csv", usecols=["product_id", "product_category_name"])
    df_sellers = load_csv("olist_sellers_dataset.csv", usecols=["seller_id", "seller_zip_code_prefix", "seller_state"])
    df_customers = load_csv("olist_customers_dataset.csv", usecols=["customer_id", "customer_unique_id", "customer_zip_code_prefix", "customer_state"])
    ...
    df_categories.rename(columns={"product_category_name": "product_category", "product_category_name_english": "product_category_english",}, inplace=True)
    df_zip.rename(columns={"geolocation_zip_code_prefix": "zip_code_prefix", "geolocation_state": "state"}, inplace=True)
    df_products.rename(columns={"product_category_name": "product_category"}, inplace=True)
    df_customers.rename(columns={"customer_zip_code_prefix": "zip_code_prefix", "customer_state": "state"}, inplace=True)
    df_sellers.rename(columns={"seller_zip_code_prefix": "zip_code_prefix", "seller_state": "state"}, inplace=True)
    ...
    df_categories.fillna({"product_category": "unindentified", "product_category_english": "unindentified"}, inplace=True)
    # id fix
    ...
    df_products.drop(columns=["product_category", "product_category_english"], inplace=True)
    # id fix
    ...
    df_customers = df_customers.merge(df_zip, on=["zip_code_prefix", "state"], how="left")
    ...
    df_orders.fillna({"approved_at": "2000-01-01 00:00:00", "delivered_carrier_date": "2000-01-01 00:00:00", "delivered_customer_date": "2000-01-01 00:00:00"}, inplace=True)
    df_orders.drop_duplicates(inplace=True)
    df_orders = df_orders.merge(df_customers, on="customer_id", how="left")
    df_orders.drop(columns=["customer_id", "customer_unique_id", "zip_code_id"], inplace=True)
    # id fix
    ...
    df_order_items = df_order_items \
        .merge(df_orders[["order_id", "oid"]], on="order_id", how="left") \
    ...
    df_order_items.rename(columns={"oid": "order_id", "pid": "product_id", "sid": "seller_id"}, inplace=True)
